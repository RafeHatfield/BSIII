...
mox.require([
	...
	mox.create('static UploadDialog', {
		...
		uploadInit : function() {
			...
			mcImageManager.execRPC('im.getConfig', {path : win.ImageManagerTheme.path}, function(r) {
				...
				if (useFlash) {
					t.swfu = new SWFUpload({
						upload_script : "../../../stream/" + mox.defaultDoc + "?cmd=im.upload&path="+ encodeURIComponent(win.ImageManagerTheme.path) +"&format=flash",
			...
			if (document.forms[0])
				document.forms[0].path.value = win.ImageManagerTheme.path;
		},
		...
		handleJSONResponse : function(o) {
			...
			if (!this.getParent().ImageManagerTheme.showStatusDialog(LangPack.get('error', 'upload_failed'), new ResultSet(o.result))) {
				t.refreshList();
				t.close();
			}
		},
		...
		onUploadSubmit : function(f) {
			if (this.getParent().ImageManagerTheme.checkDemo())
				return false;
